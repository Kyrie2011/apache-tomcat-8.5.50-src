...
public class NioEndpoint extends AbstractJsseEndpoint<NioChannel> {
    ...
    public void bind() throws Exception {
        ...
        if (!getUseInheritedChannel()) {
            // 使用java nio api的使用，创建serverSocket对象并绑定监听地址
    ...
    public void startInternal() throws Exception {
        ...
        if (!running) {
            ...
            if ( getExecutor() == null ) {
                // 创建线程池对象 (TaskQueue对象 和 ThreadPoolExecutor对象)
                createExecutor();
            }
            // 初始化最大连接数
            ...
            for (int i=0; i<pollers.length; i++) {
                ...
                pollerThread.start();  // 启动轮训线程
            }
            // 创建并启动Acceptor线程的集合 Acceptor线程是负责服务监听端口的请求接入的
    ...
    public static class PollerEvent implements Runnable {
        ...
        public void run() {
            if (interestOps == OP_REGISTER) {
                try {
                    // 取出队列中PollerEvent，重新关注OP_READ事件，并注册到Selector
    ...
    public class Poller implements Runnable {
        ...
        public boolean events() {
            ...
            PollerEvent pe = null;
            // 取出队列中新增的PollerEvent pe = events.poll()
            for (int i = 0, size = events.size(); i < size && (pe = events.poll()) != null; i++ ) {
                result = true;
                try {
                    pe.run();  // PollerEvent也是一个Runnable
                    pe.reset(); // 重置关注的事件
                    if (running && !paused) {
                        // 加入缓存队列中
                        eventCache.push(pe);
        ...
        public void register(final NioChannel socket) {
            ...
            // 添加PollerEvent对象到事件队列，SynchronizedQueue<PollerEvent> events
            addEvent(r);
        }
        ...
        public void run() {
            ...
            while (true) {
                ...
                while (iterator != null && iterator.hasNext()) {
                    SelectionKey sk = iterator.next();  // 就绪channel，对应的事件类型
                ...
                if ( close ) {
                    ...
                    if (sk.isReadable() || sk.isWritable() ) {
                        if ( attachment.getSendfileData() != null ) {
                            ...
                            if (sk.isReadable()) {  // 可读
                                // 根据选择的SelectionKey构造SocketProcessor，提交到Executor到处理
                                if (!processSocket(attachment, SocketEvent.OPEN_READ, true)) {
                                    closeSocket = true;
                                }
                            }
                            if (!closeSocket && sk.isWritable()) { // 可写
